...
function _traitsIconHelper(trait) {
    ...
    return new handlebars.SafeString('<span class="glyphicon ' + traitIcon + '" title="' + trait + '"></span>');
}
    ...
    for (var trait in is) {
        for (var traitdesc in traits) {
            if (traits[traitdesc].hasOwnProperty(is[trait])) {
                ...
                var description = traits[traitdesc][is[trait]].description;
                if (description === "TBD") {
                    description = "This endpoint is flagged as " + is[trait] + ".";
                }
...
function _showSecuredBy(auths) {
    var retval = '';
    if (auths.constructor === Array) {
        var authsLength = auths.length;
        for (var i = 0; i < authsLength; i++) {
            var auth = auths[i];
            if (auth.oauth_2_0 != null) {
                retval += "Requires one of the following OAuth2 scopes: <ul>";
                var scopes = auth.oauth_2_0.scopes;
                var scopesLength = scopes.length;
                for (var j = 0; j < scopesLength; j++) {
                    retval += "<li>" + scopes[j] + "</li>";
                }
                retval += "</ul>";
            }
        }
...
function getDefaultConfig(https, mainTemplate, resourceTemplate, itemTemplate) {
    ...
    return {
        ...
        helpers: {
            ...
            traitsIconHelper: _traitsIconHelper,
            traitsHelper: _traitsHelper,
            showSecuredBy: _showSecuredBy
        },
        partials: {
